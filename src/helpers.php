    ...
    function getColorClass(string $color = 'zinc', string $variant = '', string $context = ''): string
    {
        return match ($color) {
            ...
            'core' => match ($variant) {
                ...
                default => 'text-core-800 bg-core-100' . ($context === 'button' ? ' hover:bg-core-300' : ''),
            },
            'white' => match ($variant) {
                'solid' => 'text-core-800 bg-white',
...
if (!function_exists('twMerge')) {
    function twMerge(string $default, ?string $incoming): string
    {
        if (!$incoming) {
            return $default;
        }

        $groups = ['size-', 'text-', 'bg-', 'p-', 'px-', 'py-', 'w-', 'h-', 'rounded-'];

        $incomingPrefixes = collect(explode(' ', $incoming))
            ->filter()
            ->map(fn($c) => collect($groups)->first(fn($g) => str_starts_with($c, $g)))
            ->filter();

        $filteredDefault = collect(explode(' ', $default))
            ->reject(fn($c) => $incomingPrefixes->contains(
                collect($groups)->first(fn($g) => str_starts_with($c, $g))
            ));

        return $filteredDefault->push($incoming)->filter()->implode(' ');
    }
}
